...
def query_bank(topic: str = "list") -> str:
    ...
    전략 지식: 'walls'(열한 겹의 벽), 'identifiers'(식별자 해독), 'persons'(인명 열쇠), 'crosscheck'(국내외 대조), 'world'(6개국 서고 지도), 'cities'(도시명 계보)."""
    if topic == "list":
    ...
    if topic.strip().lower() in JB_STRAT:
        return JB_STRAT[topic.strip().lower()]
