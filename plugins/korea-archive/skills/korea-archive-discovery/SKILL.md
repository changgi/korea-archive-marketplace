...
3. 서사 우선: 발굴 경위·의미를 에세이로 서술(드롭캡 리드 문단, 풀인용 1개 이상). 문체는 담백·구체 —
   과장어(놀라운·혁신적 등)와 AI투 금지
4. 실물 이미지 필수: 게재 가능(권리 A/B + 게재윤리 1·2단계) 기록은 기관 공개 원본에서 수집해 base64로 임베드 —
   히어로 1장 + 핵심 기록마다. 기관이 비식별(블러)판을 제공하면 우선 사용. 한 장도 못 실으면 그 사유를 권리 절에 명기
...
17. 카드뉴스 병행: 보고서와 함께 insta-carousel 스킬로 캐러셀(기본 8장, 1080×1080)을 제작하고 보고서 말미
    '카드뉴스' 절에 축소 임베드 — 커버는 히어로와 같은 실물, 검증 노트·따라하기 카드 포함, 출처 대장 공유.
    PNG 원본·caption.txt·sources.txt를 보고서와 함께 납품
