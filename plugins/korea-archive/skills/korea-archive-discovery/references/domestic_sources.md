...
> v1.9.0 자동 브라우징: 국내 수집기는 서버가 각 사이트를 직접 조회해 실제 결과를 반환한다(서울정보소통광장=결재문서 목록, 전쟁기념관=카테고리 건수, 한국사DB=매칭 DB, 서울기록원=매칭 컬렉션, 국가기록원·국립중앙도서관=키 설정 시 OpenAPI). 서버 응답이 없는 곳은 에이전트 브라우저 도구로 열람.
