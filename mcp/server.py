...
국내(8): nedb_search(한국사DB) · archives_search(국가기록원) · nlk_search(국립중앙도서관)
         seoul_archives_search(서울기록원) · foia_search(정보공개포털)
         local_gov_search(지방 정보공개·기록원) · warmemo_search(전쟁기념관) · scrape_plan(폴백)
...
※ 이 파일은 개발/로컬 실행용 진입점이며, 배포본(plugins/korea-archive/servers/server.py)과
  동일한 20개 도구를 노출한다. 두 사본을 함께 갱신할 것.
"""
...
for p in (os.path.join(_ROOT, "harvester"), os.path.join(_ROOT, "keywords"),
          os.path.join(_ROOT, "collector")):
